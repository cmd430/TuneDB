	...
	//get info for item
		try {
			if( track.album !== '' && track.title !== '' ){
				if( config.verbose ) console.info( 'Grabbing Meta', track.title );
				Music.findOne({
					...
					if ( err || !doc ) {
						...
						newAlbum.save(function( err, newDocs ) {
							if( config.verbose ) console.info( 'Album \'' + track.album + '\'Saved' );
						...
						Music.findOne({
							...
							if( JSON.stringify( tracks ).indexOf( track.title ) === -1 ){
								...
								tracks.sort(function( track_a, track_b ) {
									return track_a.track.no - track_b.track.no;
								}).sort(function( track_a, track_b ) {
									return track_a.disk.no - track_b.disk.no;
								});
								Music.update({
									album: track.album
								}, {
								...
								if( config.verbose ) console.info( 'Song \'' + song + '\' Skipped' );
								return callback( null, track );
							}
						});
					}
